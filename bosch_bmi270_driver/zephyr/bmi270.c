...
int bmi270_reg_write(const struct device *dev, uint8_t reg, const uint8_t *data,
		     uint16_t length)
{
	const struct bmi270_config *cfg = dev->config;
...
int bmi270_reg_write_with_delay(const struct device *dev, uint8_t reg,
				 ...
				 uint16_t range)
{
	...
	val->val1 =
		((raw_val * (int64_t)range * SENSOR_PI) / (180LL * INT16_MAX)) /
		1000000LL;
	val->val2 =
		((raw_val * (int64_t)range * SENSOR_PI) / (180LL * INT16_MAX)) %
		1000000LL;
...
static int set_accel_odr_osr(const struct device *dev,
...
static int set_accel_range(const struct device *dev,
			   const struct sensor_value *range)
{
	struct bmi270_data *data = dev->data;
	int ret = 0;
...
static int set_gyro_odr_osr(const struct device *dev,
			    const struct sensor_value *odr,
			    const struct sensor_value *osr)
{
	struct bmi270_data *data = dev->data;
...
static int set_gyro_range(const struct device *dev,
			  const struct sensor_value *range)
{
	struct bmi270_data *data = dev->data;
	int ret = 0;
...
static int bmi270_sample_fetch(const struct device *dev,
...
static int bmi270_channel_get(const struct device *dev,
			      enum sensor_channel chan,
...
/**
 * @brief Initialization code that follows the flow chart described in the 'Testing communication and initializing the device' section, starting from page 17 of the datasheet
 */
static int bmi270_init(const struct device *dev)
{
	...
	// Read an arbitrary register of the device, discard the read response (P.25)
	...
	/* NOTE: operation must not be performed more than once after POR or soft
     reset, So always soft reset before initialize sequence (P.25) */
	...
	/* NOTE: Testing communication sequence (P.17) */
	...
	/* NOTE: Performing initialization sequence (P.18) */
	...
	/* NOTE: Checking the correct initialization status (P.19) */
...
#define BMI270_CONFIG_I2C(inst) \
	.bus.i2c = I2C_DT_SPEC_INST_GET(inst), .bus_io = &bmi270_bus_io_i2c,
...
#define BMI270_CREATE_INST(inst)                                         \
	...
	static const struct bmi270_config bmi270_config_##inst = {       \
		...
		BMI270_CONFIG_INT(inst)                                  \
                                                                         ...
                                                                         \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, bmi270_init, NULL,            \
				     &bmi270_drv_##inst,                 \
